...
int n; 
int m;
gboolean isGameInterfaceActive = FALSE;
...
void show_main_window(int *client_socket_pt) {
    ...
    g_signal_connect(start_btn, "clicked", G_CALLBACK(on_start_btn_clicked), client_socket_pt);
...
void start_game_interface(Package *pkg){
	builder = gtk_builder_new_from_file("../views/game_interface.glade");
    game = GTK_WIDGET(gtk_builder_get_object(builder, "game"));
    game_interface = GTK_WIDGET(gtk_builder_get_object(builder, "game_interface"));
    question = GTK_WIDGET(gtk_builder_get_object(builder, "question"));
    answerA = GTK_WIDGET(gtk_builder_get_object(builder, "answerA"));
